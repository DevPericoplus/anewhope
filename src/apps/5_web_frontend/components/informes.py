"""
Componente de Informes para el frontend.
...
class InformesState(rx.State):
    ...
    # ID de organización (viene de sesión en frontend)
    ...
    async def load_proyectos(self):
        ...
        try:
            ...
            print(f"[ERROR INFORMES FRONTEND] Error al cargar proyectos: {e}")
            import traceback
            traceback.print_exc()
    ...
    async def load_versiones(self):
        ...
        try:
            ...
            print(f"[DEBUG INFORMES FRONTEND] Versiones obtenidas: {len(versiones)}")
            for v in versiones:
    ...
    async def load_archivos(self):
        ...
        try:
            ...
            print(f"[ERROR INFORMES FRONTEND] Error al cargar archivos: {e}")
            import traceback
            traceback.print_exc()
    ...
    async def load_markdown_content(self):
        ...
        try:
            ...
            if content:
                ...
                print(f"[DEBUG INFORMES FRONTEND] Contenido cargado: {len(content)} caracteres")
            else:
                self.markdown_content = ""
                print("[ERROR INFORMES FRONTEND] No se pudo cargar el contenido")

        except Exception as e:
            print(f"[ERROR INFORMES FRONTEND] Error al cargar contenido markdown: {e}")
            import traceback
            traceback.print_exc()
    ...
    async def on_mount_informes(self):
        ...
        try:
            from web_frontend.web_frontend import State as MainState
            main_state = await self.get_state(MainState)
            self.organization_id = main_state.organization_id
            print(f"[DEBUG INFORMES FRONTEND] organization_id={self.organization_id}")
            ...
            print(f"[ERROR INFORMES FRONTEND] Error en on_mount: {e}")
            import traceback
            traceback.print_exc()
...
def informes_panel() -> rx.Component:
    ...
    return rx.vstack(
        # Título
        rx.heading("Informes", size="8", color=COLORS["accent"], margin_top="0", margin_bottom="0.3em"),
